...
public class LogConfigurator {
	...
	private void configureFileAppender() {
		...
		try {
			rollingFileAppender = new RollingFileAppender(fileLayout, getFileName() + ".log");
		...
		final MultiLevelRollingFileAppender multiLevelRollingFileAppender;
		final Layout fileLayout1 = new PatternLayout(getFilePattern());

		try {
			multiLevelRollingFileAppender = new MultiLevelRollingFileAppender(fileLayout1, getFileName() + "_err.log");
		} catch (final IOException e) {
			throw new RuntimeException("Exception configuring log system", e);
		}

		multiLevelRollingFileAppender.setMaxBackupIndex(getMaxBackupSize());
		multiLevelRollingFileAppender.setMaximumFileSize(getMaxFileSize());
		multiLevelRollingFileAppender.setImmediateFlush(isImmediateFlush());

		Priority[] priorities1 = new Priority[]{Priority.ERROR, Priority.FATAL};
		multiLevelRollingFileAppender.setThresholds(priorities1);
		root.addAppender(multiLevelRollingFileAppender);
